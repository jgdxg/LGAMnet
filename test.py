...
from net.comper import unet,unet++,attentionnet,R2Unet,RIUnet,QAU_Net,DefEDNet
import joblib
import copy
...
def main():
    ...
    args = joblib.load('models/LiTS_Unet_lym/defnet/args.pkl')
    print('Config -----')
    for arg in vars(args):
        print('%s: %s' %(arg, getattr(args, arg)))
    print('------------')
    joblib.dump(args, 'models/LiTS_Unet_lym/defnet/args.pkl')
    # joblib.dump(args, 'models/LiTS_Unet_lym/unet++/args.pkl')

    # create model
    print("=> creating model %s" %args.arch)
    print(torch.cuda.is_available())
    model = defnet.DefEDNet()
    model = torch.nn.DataParallel(model).cuda()

    model.load_state_dict(torch.load('models/LiTS/defnet/epoch145-0.9721-0.8657_model.pth'))
    model.eval()
    ...
    for file_index, file in enumerate(os.listdir(test_ct_path)):
        ...
        print(file, "liver predict finish")
        ...
        print(file, "tumor predict finish")

        print('-----------------------')

    timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    result_path = '{}/result_{}_{}.xlsx'.format(result_path1, model_name, timestamp)
