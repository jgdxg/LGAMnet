...
from utilities.metrics import dice_coef, batch_iou, mean_iou, iou_score,calVOE,calRVD
...
#换模型需要修改的地方
arch_names = list(snpt1.__dict__.keys())
...
def parse_args():
    ...
    #换模型需要修改的地方
    parser.add_argument('--arch', '-a', metavar='ARCH', default='fct',
                        choices=arch_names,
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        voe1 = calVOE(output,target)[0]
        voe2 = calVOE(output,target)[1]
        voe1 = np.float64(voe1)
        voe2 = np.float64(voe2)

        rvd_1 = calRVD(output, target)[0]
        rvd_2 = calRVD(output, target)[1]
        rvd_1 = np.float64(rvd_1)
        rvd_2 = np.float64(rvd_2)
        ...
        voe1s.update(voe1,input.size(0))
        voe2s.update(voe2, input.size(0))



        rvd_1s.update(rvd_1, input.size(0))
        rvd_2s.update(rvd_2, input.size(0))
...
def validate(args, val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (input, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            # asd1s.update(asd1, input.size(0))
            # asd2s.update(asd2, input.size(0))
...
def main():
    ...
    # Data loading code
    img_paths = glob('./bata/trainImage_k1_1217/*')
    mask_paths = glob('./bata/trainMask_k1_1217/*')
    ...
    log = pd.DataFrame(index=[], columns=[
        'epoch', 'lr', 'loss', 'iou','dice_1', 'dice_2','voe1','voe2','rvd_1','rvd_2','val_loss', 'val_iou','val_dice_1', 'val_dice_2','val_voe1','val_voe2','val_rvd_1','val_rvd_2'
    ])
    ...
    for epoch in range(args.epochs):
        ...
        print('voe1 %.4f - voe2 %.4f - rvd_1 %.4f - rvd_2 %.4f' % (train_log['voe1'],train_log['voe2'],train_log['rvd_1'],train_log['rvd_2']))
        ...
        tmp = pd.Series([
            ...
            val_log['rvd_1'],
            val_log['rvd_2'],
        ], index=['epoch', 'lr', 'loss', 'iou', 'dice_1' ,'dice_2' ,'voe1','voe2','rvd_1','rvd_2','val_loss', 'val_iou', 'val_dice_1' ,'val_dice_2','val_voe1','val_voe2','val_rvd_1','val_rvd_2'])

        log = log.append(tmp, ignore_index=True)
        # log = log.concat(tmp, ignore_index=True)
